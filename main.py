...
def _normalise_scrip(row) -> dict:
    """
    Normalises a scrip entry to guaranteed dict with 'token' and 'symbol' keys.

    gap_scanner.ScripMaster.load() returns dict values like:
      {"token": "40", "symbol": "AEGISLOG", "series": "EQ", "name": "...", ...}
    These are already correct — just ensure key names are consistent.

    If somehow a plain string arrives (e.g. old watchlist format), wrap it safely.
    """
    if isinstance(row, dict):
        token  = str(row.get("token")  or row.get("TOKEN")  or
                     row.get("instrument_token") or row.get("pSymbol") or "").strip()
        symbol = str(row.get("symbol") or row.get("SYMBOL") or
                     row.get("trading_symbol")  or row.get("pSymbolName") or "").strip().upper()
        isin   = str(row.get("isin")   or row.get("ISIN")   or
                     row.get("pISIN")  or "").strip()
        name   = str(row.get("name")   or row.get("NAME")   or
                     row.get("pDesc")  or symbol).strip()
        return {"token": token, "symbol": symbol, "isin": isin, "name": name}
    elif isinstance(row, str):
        # Plain string — treat as symbol
        sym = row.strip().upper()
        return {"token": "", "symbol": sym, "isin": "", "name": sym}
    else:
        return {"token": "", "symbol": str(row).upper(), "isin": "", "name": ""}
...
class GapVWAPAlgo:
    ...
    def initialize(self):
        ...
        # raw_scrips is a dict {symbol: info_dict} — must iterate .values()
        # to get the info dicts, NOT the keys (which are plain symbol strings).
        scrips_iter = raw_scrips.values() if isinstance(raw_scrips, dict) else raw_scrips
        self._all_scrips = [_normalise_scrip(r) for r in scrips_iter]
    ...
    def run_gap_scan(self):
        ...
        if is_first_scan:
            # Seed VWAP for all gap stocks from session data
            # This is critical when algo starts late (e.g. 10:31)
            # Without seeding, self-compute VWAP starts from scratch
            # and gives wrong values (e.g. AEGISLOG: algo=701, real=715)
            self._seed_gap_vwaps(all_gap_stocks)
    ...
    def _check_entry_signal(self, token: str, ltp: float):
        ...
        # Pre-screen without lock — avoids lock contention on every tick
        ...
        vwap_src = "atp✓" if tracker._seeded else "self-compute⚠️"

        self.logger.info(f"[Signal] {symbol} {direction} [{sig_type}]  "
                         f"LTP={ltp:.2f} VWAP={vwap:.2f} [{vwap_src}]  slope={slope:+.4f}%/min")
        print(f"\n🔔 SIGNAL: {direction} {symbol}  [{sig_type}]")
        print(f"   LTP ₹{ltp:.2f}  VWAP ₹{vwap:.2f} [{vwap_src}]  "
              f"Dist {abs(ltp-vwap)/vwap*100:.2f}%  slope {slope:+.4f}%/min")

        # B-02 FIX: use try_enter (atomic lock + re-check + enter)
        # Replaces the old separate can_enter() → enter() sequence
        trade = self.trade_mgr.try_enter(
            symbol        = symbol,
            token         = token,
            direction     = direction,
            ltp           = ltp,
    ...
    # ─────────────────────────────────────────────────────
    #  VWAP Seeding — fetch real session VWAP at startup
    # ─────────────────────────────────────────────────────

    def _seed_gap_vwaps(self, gap_stocks: list):
        """
        Fetches real session VWAP (atp) for all gap stocks at startup.

        WHY THIS IS NEEDED:
        When algo starts late (e.g. 10:31), the self-compute VWAP starts
        from scratch — it only sees ticks from 10:31 onwards, missing
        1h15m of session data from 9:15. This gives completely wrong VWAP.

        Example today:
          AEGISLOG real VWAP (TradingView) = 715.48
          Algo self-computed VWAP          = 701.02  (only 7 min of data)
          Error = 14 rupees → wrong SL, wrong signal level

        FIX: After subscribing gap stocks, fetch a market depth or full
        quote for each stock. Kotak Neo /depth endpoint returns 'atp'
        (average traded price = session VWAP). Use this to seed each tracker.

        After seeding, WS ticks with 'atp' will keep it accurate in real-time.
        """
        if not gap_stocks:
            return

        t = now_ist()
        if t.time() <= datetime.time(9, 16):
            # Market just opened — no late-start issue, no seeding needed
            return

        print(f"\n[VWAPSeed] Seeding session VWAP for {len(gap_stocks)} gap stocks...")

        tokens = [
            {"instrument_token": s["token"], "exchange_segment": config.CM_SEGMENT}
            for s in gap_stocks
            if s.get("token")
        ]
        if not tokens:
        ...
        seeded = 0
        try:
            # Use quote_type "depth" — this returns atp in Kotak Neo API
            for i in range(0, len(tokens), 25):   # depth is heavier, use batch of 25
                batch = tokens[i:i+25]
                try:
                    ...
                    for item in items:
                        ...
                        # Extract atp — Kotak Neo depth response
                        atp = 0.0
                        ...
                        if atp <= 0:
                            for nested_key in ("depth", "marketDepth", "quote"):
                        ...
                        vol = 0.0
                        for f in ("v", "vol", "volume", "ttv", "trdVol"):
                            v = item.get(f)
                            if v:
                                try:
                                    vol = float(v)
                        ...
                        if atp > 0:
                            self.vwap_mgr.seed_stock_vwap_by_token(tok, atp, vol)
                            seeded += 1

                    time.sleep(0.5)
                except Exception as e:
                    self.logger.warning(f"[VWAPSeed] Batch error: {e}")

        except Exception as e:
            self.logger.error(f"[VWAPSeed] Failed: {e}")

        print(f"[VWAPSeed] Seeded {seeded}/{len(gap_stocks)} stocks  "
              f"({'✅ OK' if seeded > 0 else '⚠️  0 seeded — WS atp will build up'})")
    ...
    def _rest_poll_loop(self):
        """
        REST poll every 15s for gap stocks.
        Uses quote_type='depth' to get the 'atp' field (session VWAP).
        The 'ltp' endpoint does NOT return atp in Kotak Neo.
        WS is primary source; REST is fallback backup between ticks.
        """
        print("[REST] Gap poll loop started — every 15s  (depth quotes → atp)")
        while self._running:
            ...
            try:
                for i in range(0, len(instruments), 25):  # depth is heavier — 25 per batch
                    resp  = self.client.quotes(
                        instrument_tokens=instruments[i:i+25],
                        quote_type="depth",   # returns atp (session VWAP)
                    )
                    items = (resp.get("data", resp.get("success", []))
                             if isinstance(resp, dict) else resp
                             if isinstance(resp, list) else [])
                    ...
                    for item in items:
                        ...
                        atp_val = self._extract_atp(item)   # atp = session VWAP
                        if not tok or ltp_val <= 0:
                            continue

                        # Use "atp" key in synthetic tick — matches vwap_engine priority
                        self.vwap_mgr.on_tick(tok, {"ltp": ltp_val, "atp": atp_val},
                                              from_ws=False)
    ...
    def _rest_trend_scan_loop(self):
        ...
        while self._running:
            ...
            scanned = 0
            try:
                # Trend scan uses ltp (lighter) — atp not critical here because
                # trend signals need 20 bars to confirm, not single-tick precision.
                # Self-compute VWAP accumulates correctly across 60s REST updates.
                # For gap stocks (already subscribed), WS provides accurate atp.
                for i in range(0, len(instruments), 50):
                    ...
                    for item in items:
                        ...
                        # No atp from /ltp endpoint — self-compute VWAP accumulates
                        # correctly across many 60s ticks for trend detection
                        self.vwap_mgr.on_tick(tok, {"ltp": ltp_val},
                                              from_ws=False)
    ...
    def _extract_atp(self, item: dict) -> float:
        """
        Extract average traded price (= session VWAP) from a REST quote response.
        Kotak Neo depth response field: 'atp'
        Also checks common aliases for other response formats.
        """
        for f in ("atp", "aP", "ap", "avg_price", "averageTradedPrice", "average_price"):
            v = item.get(f)
            if v and not isinstance(v, dict):
                try:
                    val = float(v)
                    ...
                    pass
        # Try nested depth/quote structures
        for nested_key in ("depth", "marketDepth", "quote"):
            nested = item.get(nested_key)
            if isinstance(nested, dict):
                for f in ("atp", "aP", "ap"):
    ...
    # Keep _extract_ap as alias for backward compat
    def _extract_ap(self, item: dict) -> float:
        return self._extract_atp(item)
    ...
    def _on_reconnect(self, new_client):
        ...
        self.trade_mgr.order_mgr.client      = new_client   # keep order manager in sync
